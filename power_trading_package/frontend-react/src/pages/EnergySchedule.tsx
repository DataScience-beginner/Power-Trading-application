...
const EnergySchedule: FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'hourly' | 'daily' | 'weekly' | 'monthly'>('daily');
  ...
  // Get current month summary - find the one with the most days completed (in case of multiple portfolios)
  const currentMonthData = months
    .filter((m) => m.month === selectedMonth && m.year === selectedYear)
    .sort((a, b) => b.total_days_completed - a.total_days_completed)[0];

  // Available years from months data
  const availableYears = Array.from(new Set(months.map((m) => m.year))).sort((a, b) => b - a);

  // Available months for selected year - group by month and show max days
  const monthsGrouped = months
    .filter((m) => m.year === selectedYear)
    .reduce((acc, m) => {
      const existing = acc.find(item => item.month === m.month);
      if (!existing || m.total_days_completed > existing.total_days_completed) {
        return [...acc.filter(item => item.month !== m.month), m];
      }
      return acc;
    }, [] as EnergyScheduleMonth[])
    .sort((a, b) => a.month - b.month);
  
  const availableMonths = monthsGrouped;
    ...
    <Box>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <FormControl size="small" sx={{ minWidth: 120 }}>
            ...
            <Select
              value={viewMode}
              label="View"
              onChange={(e) => setViewMode(e.target.value as 'hourly' | 'daily' | 'weekly' | 'monthly')}
            >
      ...
      {loading && selectedMonth === '' ? (
        ...
        <>
          ...
          {/* Monthly Trend Chart */}
          <Box sx={{ mb: 3 }}>
            <MonthlyTrendChart days={days} viewMode={viewMode} />
          </Box>
