...
async def calculate_energy_schedule(
    ...
    """
    Calculate energy schedule for a specific date
    
    Complete workflow:
    1. Validate DOR (previous day) and SCH (current day) files exist
    2. Get or create monthly Excel file
    3. Populate Excel with data from database
    4. Read calculated results from Excel formulas
    ...
    try:
        ...
        if calculation_date:
            try:
                calc_date = datetime.fromisoformat(calculation_date).date()
            except:
                # Try parsing just the date part
                calc_date = datetime.strptime(calculation_date.split('T')[0], '%Y-%m-%d').date()
        else:
    ...
    try:
        ...
        for sheet in month_sheets:
            result.append({
                ...
                "total_energy_savings_mwh": sheet.total_energy_savings_mwh,
                "total_gdam_cost": sheet.total_gdam_cost,
                "total_dam_cost": sheet.total_dam_cost,
                "total_rtm_cost": sheet.total_rtm_cost,
                "average_ctu_losses_percent": sheet.average_ctu_losses_percent,
                "days_completed": sheet.days_completed,
